...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    if (!email.toLowerCase().endsWith('@epn.edu.ec')) {
      setMensaje({ 
        texto: 'Solo se permiten correos institucionales @epn.edu.ec. Si eres visitante, usa "Ingresar con Google".', 
        tipo: 'error' 
      });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        if (data.uid) {
          ...
          setMensaje({ 
            texto: 'Error: El servidor no envió el ID de usuario.', 
            tipo: 'error' 
          });
        }
      } else {
        setMensaje({ 
          texto: data.mensaje || 'Credenciales incorrectas', 
          tipo: 'error' 
        });
      }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (res) {
        ...
        setMensaje({ 
          texto: 'No se pudo iniciar sesión con Google', 
          tipo: 'error' 
        });
      }
        ...
        <form onSubmit={handleLogin} className="max-w-sm w-full flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center justify-between">
              ...
              <Link 
                to="/forgot-password" 
                className="text-xs text-neutral-muted hover:text-brand-primary transition-colors"
              >
          ...
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input 
              type="checkbox" 
              className="w-4 h-4 rounded border-neutral-border accent-brand-primary cursor-pointer" 
            />
          ...
          <p className="text-center text-sm text-neutral-muted">
            ...
            <Link
              to="/register"
              className="text-brand-accent font-semibold hover:opacity-80 transition-opacity"
            >
              Registrarse
            </Link>
          </p>

          <p className="text-center text-xs text-neutral-subtle">
